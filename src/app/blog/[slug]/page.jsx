...
const SinglePostPage = async ({ params }) => {
    ...
    return (
        <article className="items-start min-h-[45rem] w-fit dark:bg-black bg-white  dark:bg-grid-white/[0.1] bg-grid-black/[0.1] relative justify-center">
            ...
            <div className='flex lg:flex-row flex-col gap-6 px-0 sm:px-12  items-start'>
                ...
                <div className='flex flex-col gap-3'>
                    ...
                    <h2 className='font-bold text-2xl sm:text-4xl max-w-2xl relative'>{post?.title}</h2>
                    {(post && <Suspense fallback={<div><ImSpinner8 className=' min-h-16 animate-spin text-zinc-400 text-2xl mx-12' /></div>}>
                        <PostUser userId={post.userId} />
                    </Suspense>)}
                    <div className='flex flex-col gap-3 h-fit relative my-3 text-sm sm:text-md pr-0 sm:pr-6'>
                        {post?.description}
                    </div>
                </div>
            </div>
